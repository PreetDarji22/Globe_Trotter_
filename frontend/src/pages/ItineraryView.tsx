...
import { Clock, MapPin, Coffee, Camera, Bed, Plane, Plus } from 'lucide-react';
import { SafeImage } from '../components/SafeImage';

export default function ItineraryView() {
  const { id } = useParams();
  const { currentTrip, fetchTripDetails, addActivity } = useStore() as any;
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-4 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 mt-8 bg-white p-6 rounded-3xl shadow-sm border border-stone-200">
        <div className="flex items-center gap-6 mb-6 md:mb-0">
          <SafeImage src={currentTrip.coverPhoto || 'https://images.unsplash.com/photo-1488085061387-422e29b40080?auto=format&fit=crop&q=80&w=800'} className="w-24 h-24 rounded-2xl object-cover shadow-sm" alt={currentTrip.name} />
          <div>
            <h2 className="font-display font-black text-3xl text-slate">{currentTrip.name}</h2>
            <p className="text-stone-500 font-medium flex items-center gap-1.5 mt-1"><MapPin size={16}/> {currentTrip.description || 'Custom Trip'}</p>
          </div>
        </div>
        
        <div className="flex gap-8 w-full md:w-auto md:border-l border-stone-200 md:pl-8">
          <div className="flex-1 md:flex-none">
            <p className="text-xs font-bold text-stone-400 uppercase tracking-widest mb-2">Cost Breakdown</p>
            <div className="flex flex-wrap gap-2 max-w-[200px]">
              {Object.entries(categoryCosts).map(([type, cost]) => (
                <span key={type} className="text-xs font-bold bg-stone-100 px-2 py-1 rounded text-slate">
                  {type}: ${cost as number}
                </span>
              ))}
              {Object.keys(categoryCosts).length === 0 && <span className="text-xs text-stone-400">No expenses yet</span>}
            </div>
          </div>
          <div className="text-right flex-1 md:flex-none">
            <p className="text-xs font-bold text-stone-400 uppercase tracking-widest mb-1">Total Expense</p>
            <p className="text-3xl font-black text-genz-green">${totalCost.toFixed(2)}</p>
          </div>
        </div>
      </div>
